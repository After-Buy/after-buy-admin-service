 ...
 *
 * 보안: X-Internal-Secret 헤더 검증으로 외부 접근 차단
...
public class InternalErrorLogController {
	 ...
	 *
	 * @param secret  : X-Internal-Secret 헤더 (MSA 내부 인증)
	...
	public ResponseEntity<Map<String, Boolean>> receiveErrorLog(
		...
		/* X-Internal-Secret 헤더 검증 — 불일치 시 401 반환 */
		if (!internalSecretKey.equals(secret)) {
			log.warn("[InternalErrorLogController] X-Internal-Secret 불일치: service={}", request.getServiceName());
